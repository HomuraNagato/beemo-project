...
usage() {
  cat <<'USAGE'
usage: beemo-start.sh [vllm-gpu|vllm-cpu|llama-cpu] [--no-voice] [--restart-orchestrator]

Profiles:
  vllm-gpu   vLLM reasoning and embedding on NVIDIA GPU
  vllm-cpu   vLLM reasoning and embedding on CPU
  llama-cpu  llama.cpp reasoning with vLLM embedding on CPU
USAGE
}
...
compose() {
  docker compose "${BEEMO_COMPOSE_FILES[@]}" "$@"
}
...
printf 'Starting Beemo services using %s profile\n' "$BEEMO_PROFILE_NAME"
