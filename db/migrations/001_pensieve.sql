...
CREATE TABLE IF NOT EXISTS subject_aliases (
    ...
    alias TEXT NOT NULL,
    created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
    PRIMARY KEY (session_id, subject_id, alias),
    FOREIGN KEY (session_id, subject_id) REFERENCES subjects(session_id, subject_id) ON DELETE CASCADE
);
...
CREATE INDEX IF NOT EXISTS idx_observations_subject_attr_created
    ON observations (session_id, subject_id, attribute, created_at DESC, id DESC);

CREATE INDEX IF NOT EXISTS idx_observations_subject_route_created
    ON observations (session_id, subject_id, route, created_at DESC, id DESC);
